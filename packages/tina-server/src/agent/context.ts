...
export class ContextBuilder {
  ...
  private buildIdentity(): string {
    ...
    return [
      ...
      'You can read, write, edit, and delete files inside the workspace, run shell commands, fetch web pages, manage long-term memory through dedicated memory tools, and send messages back to the current chat.',
      '',
      '## Current Time',
      now,
      '',
      ...
      '- All file and shell operations are confined to the workspace directory.',
      '- Shell commands run via run_shell. On Windows, commands execute in PowerShell.',
      `- Shell commands have a default 60-second timeout. Use the timeout parameter for longer operations.`,
      '- You can set up a Python virtual environment: run_shell with `python -m venv .venv`, then use `.venv/Scripts/pip` (Windows) or `.venv/bin/pip` (Unix) to install packages.',
      '- You can initialize Git: run_shell with `git init` and use standard git commands.',
      '- Use write_file to create new files. Use edit_file for targeted replacements. Use delete_file to remove files.',
